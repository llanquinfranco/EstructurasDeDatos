...
public class HeapMax {
        ...
        while (!salir) {
            ...
            if (posPadre >= 1) {
                ...
                if (this.heap[posPadre].compareTo(aux) > 0) {
                    // si el padre es mayor que sus hijos, esta bien ubicado
                    salir = true;
                } else {
                    // si el hijo es mayor que el padre, los intercambia
    ...
    public boolean eliminarCima() {
        ...
        if (this.ultimo == 0) {
            ...
            this.ultimo--;
            // Hacemos cumplir la propiedad de heap maximo
            hacerBajar(1);
            exito = true;
        }
        return exito;
    ...
    private void hacerBajar(int posPadre) {
        ...
        while (!salir) {
            ...
            if (posHijo <= this.ultimo) {
                // aux tiene al menos un hijo (izq) y lo considera mayor
                if (posHijo < this.ultimo) {
    ...
    public HeapMax clone() {
        HeapMax heapMaxClon = new HeapMax();
        int i = 0;
        while(i <= this.ultimo) {
            heapMaxClon.heap[i] = this.heap[i];
            i++;
        }
        heapMaxClon.ultimo = this.ultimo;
        return heapMaxClon;
    }
