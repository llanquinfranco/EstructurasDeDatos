...
public class MapeoAMuchos {
    ...
    private boolean desasociarAux(NodoAVLMapeoM nodo, NodoAVLMapeoM padre, Comparable valorDominio, Object valorRango) {
        ...
        if (nodo != null) {
            if (valorDominio.compareTo(nodo.getDominio()) == 0) {
                exito = eliminarRango(nodo.getRango(), valorRango);
                if(exito) {
    ...
    private boolean eliminarRango(Lista lista, Object valorRango) {
        boolean exito = lista.eliminarSolicitud(valorRango);
        return exito;
    }
